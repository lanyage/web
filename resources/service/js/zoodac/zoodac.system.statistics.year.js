...
var System = {
	init : function(){
		...
		html += '	<select id="year"></select><span class="yearText">'+$.rtls.system.form.year+'</span>';
		...
		$("#but-search").button({
			...
			System.getSystemChartYear();
			return false; 
		});
		
		System.getSystemChartYear();
	},
	getSystemChartYear : function(){
		$.ajax({
			async : true,
			type: 'GET',
			url: "/service/system.json?action=get.system.chart.year",
					...
					for(var j=1; j <= 12; j++){
						...
						for(var k=0; k < cpu.items.length; k++){
							if(j == parseInt(cpu.items[k].atime)){
								check = true;
								cpuDatas.push({x : Date.UTC(year, j-1, 1, 0, 0), y : parseFloat(cpu.items[k].percent)});
								break;
							}
							
						}
						if(!check){
							cpuDatas.push({x : Date.UTC(year, j-1, 1, 0, 0), y : 0});
						}
					...
					for(var j=1; j <= 12; j++){
						...
						for(var k=0; k < memory.items.length; k++){
							if(j == parseInt(memory.items[k].atime)){
								check = true;
								memoryDatas.push({x : Date.UTC(year, j-1, 1, 0, 0), y : parseFloat(memory.items[k].percent)});
								break;
							}
							
						}
						if(!check){
							memoryDatas.push({x : Date.UTC(year, j-1, 1, 0, 0), y : 0});
						}
					...
					for(var j=1; j <= 12; j++){
						...
						for(var k=0; k < network.items.length; k++){
							if(j == parseInt(network.items[k].atime)){
								check = true;
								networkDatas.push({x : Date.UTC(year, j-1, 1, 0, 0), y : parseFloat(network.items[k].bytesPerMin)});
								break;
							}
							
						}
						if(!check){
							networkDatas.push({x : Date.UTC(year, j-1, 1, 0, 0), y : 0});
						}
