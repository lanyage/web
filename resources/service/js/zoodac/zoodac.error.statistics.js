	...
	getPlans : function(){
		$.ajax({
            ...
            success : function (data) {
            	...
            	var item;
				$('#splan-selectable').append("<li class='map-li'><i id='all' class='icon-list-ul'></i> "+$.rtls.service.form.full+"</li>");
            	for(var i=0; i < Error.$plans.length; i++){
					item = Error.$plans[i];
					$('#splan-selectable').append("<li class='map-li'><i id='all' class='icon-list-ul'></i> " +item.name+"</li>");
				}
            	$("#splan-selectable").selectable({
        			stop: function() {
        				$( ".map-li-active", this ).each(function() {
