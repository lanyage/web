...
(function ($) {
	$.addFlex = function (t, p) {
		...
		p = $.extend({ //apply default properties
			...
			page: 1, //current page
			total: 1, //total pages
			totalNum: 1, //total num 
			useRp: true, //use the results per page select box
			rp: 15, //results per page
			rpOptions: [10, 15, 20, 30, 50], //allowed per-page values 
			title: false,
			pagestat: '총 {totalNum} 건 / {from} ~ {to} page',
			pagetext: 'Page',
			outof: 'of',
			findtext: '검색',
			procmsg: '처리중, 잠시 기다리세요',
			query: '',
			qtype: '',
			nomsg: '해당 자료가 없습니다',
		...
		var g = {
			...
			addData: function (data) { //parse data
				if (p.dataType == 'json') {
					data = $.extend({rows: [], page: 0, total: 0, totalNum : 0}, data);
				}
				if (p.preProcess) {
			...
			buildpager: function () { //rebuild pager based on new properties
				...
				stat = stat.replace(/{totalNum}/, p.totalNum);
