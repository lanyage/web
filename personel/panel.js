...
//			}
    		...
    		*/
			...
			for(var i=0, tag; tag = tags[i]; i++){
				...
				if(tag.type == 1){
					html += '<li id="tag_'+tag.euid+'"><img src="/resources/commons/images/map/icon_tag_fixed.png" style="vertical-align: middle;padding-right:4px"/>'+tag.euid+'<i class="icon-pause icon-large" style="display:none"></i><i class="icon-camera icon-large" style="display:none"></i></li>';
        		}else if(tag.type == 2){
					html += '<li id="tag_'+tag.euid+'"><img src="/resources/commons/images/map/icon_tag_move.png"  style="vertical-align: middle;padding-right:4px"/>'+tag.euid+'<i class="icon-pause icon-large" style="display:none"></i><i class="icon-camera icon-large" style="display:none"></i></li>';
        		}else{
					html += '<li id="tag_'+tag.euid+'"><img src="/resources/commons/images/map/icon_tag_card.png"  style="vertical-align: middle;padding-right:4px"/>'+tag.euid+'<i class="icon-pause icon-large" style="display:none"></i><i class="icon-camera icon-large" style="display:none"></i></li>';
        		}
			...
			$("#tags li i:odd").click(function(){
			    ...
			    $.ajax({
				    type : 'GET',
				    url : "/sendTagEuid?tagEuid="+tagLable,
				    contentType: "application/json; charset=utf-8",
				    data : {
					    "tagEuid" : tagLable
				    }
	...
	window : {
		...
		open : function(planId, name, index){ //展开最大化
			var url = "/service/panel.action?pages=service.panel&planId="+planId+"&index="+index;
	...
	notify : {
		init : function(){
		    ...
		    client.connect({}, function(frame) {
				...
				client.subscribe("/queue/alarm", function(message) {
		    		if(Panel.$isInit){
